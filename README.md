# Oregon_narodmon
Скетч для передачи показаний с беспроводных датчиков Oregon Scientific (термогигрометр, барометр, анемометр, счётчик осадков, измеритель UVI) на сервис «Народный мониторинг» (narodmon.ru)
с помощью Arduino-совместимых плат на основе ESP8266 (Wemos D1, NodeMCU).

Для подключения необходимы:
- Сам датчик,
- Arduino-совместимая плата на основе ESP8266 (Wemos D1 или NodeMCU),
- Приёмник OOK 433 МГц (питание 3 В, подключается к D7),
- Wi-Fi подключение к Интернету,
- Arduino IDE с установленной поддержкой ESP8266-совместимых устройств и библиотекой Oregon_NR.
